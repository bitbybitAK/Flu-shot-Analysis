...
def create_county_trends_chart():
    ...
    # Color palette for different counties
    ...
    for i, county in enumerate(counties):
        ...
        if len(county_data) > 1:  # Only plot counties with multiple data points
            ...
            fig.add_trace(go.Scatter(
                ...
                showlegend=True if i < 20 else False  # Limit legend to first 20 counties
            ))
    ...
    fig.update_layout(
        title={
            'text': 'Flu Vaccination Rates by County Over Time<br><sub>With 95% Confidence Intervals</sub>',
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 20}
        },
...
if __name__ == "__main__":
    ...
    print("  - county_trends_comprehensive.html (all counties)")
